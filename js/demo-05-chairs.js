(function () {
  ...
  function updateAIChairRescue(survivor, dt, now, hunterDistance) {
    if (!isAISafeToRescueChair(survivor, hunterDistance)) return false;
  ...
  function isAISafeToRescueChair(survivor, hunterDistance) {
    if (survivor.action || survivor.state !== "healthy" && survivor.state !== "injured") return false;
    if (hunter.target === survivor) return false;
    if (hunterDistance < 220) return false;
    return true;
  }
  ...
  function updateAIHunterChairing(dt, now) {
    if (hunter.carrying) {
      ...
      moveActorToPoint(hunter, chairTarget.x, chairTarget.y, hunter.speed * 0.82, dt, now);
      updateCarriedSurvivorPosition();
      return true;
    }
  ...
  function finishRescue(action) {
    ...
    target.action = null;
    target.chair = null;
    target.boostUntil = performance.now() + 1200;
    chair.survivor = null;
  ...
  function drawSurvivorLabel(survivor) {
    ...
    else if (survivor.state === "carried") label = `${survivor.name} 牵起`;
