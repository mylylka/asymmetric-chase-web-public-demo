(function () {
  ...
  function isAISafeForObjective(survivor, hunterDistance) {
    if (survivor.action || survivor.state === "downed" || survivor.escaped) return false;
    if (hunter.target === survivor) return false;
  ...
  function handlePlayerUse(now) {
    ...
    if (player.state === "downed" || player.action || player.escaped) return;
  ...
  function startHealing(healer, target, now) {
    if (healer.action || !canBeHealed(target) || healer === target) return;
    healer.action = {
      kind: "healing",
      start: now,
      target,
  ...
  function drawPlayer() {
    ...
    if (player.action && ["repairing", "openingGate", "escaping"].includes(player.action.kind)) {
  ...
  function drawTeammates() {
    teammates.forEach((survivor) => {
      if (!survivor.escaped) drawSurvivor(survivor, false);
    });
  }
  ...
  function drawSurvivorLabel(survivor) {
    ...
    ctx.fillStyle = survivor.action && ["beingHealed", "repairing", "openingGate", "escaping"].includes(survivor.action.kind)
      ? "#b7d6c1"
      : survivor.state === "healthy" ? "rgba(16, 22, 18, 0.86)" : survivor.state === "injured" ? "#d9b76a" : "#b95f52";
    ctx.beginPath();
    ctx.roundRect(-40, -9, 80, 18, 6);
    ctx.fill();
    ctx.fillStyle = survivor.state === "healthy" && (!survivor.action || !["beingHealed", "repairing", "openingGate", "escaping"].includes(survivor.action.kind)) ? "#eef3ed" : "#101611";
  ...
  function updateReadouts() {
    ...
    if (escapeReadout) escapeReadout.textContent = `${getSurvivors().filter((survivor) => survivor.escaped).length}/4`;
  ...
  function checkMatchResult() {
    ...
    const activeSurvivors = survivors.filter((survivor) => !survivor.escaped);
    ...
    if (activeSurvivors.length > 0 && activeSurvivors.every((survivor) => survivor.state === "downed")) {
      if (escapedCount === 2) {
