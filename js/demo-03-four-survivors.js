(function () {
  ...
  function keyToInput(key, value) {
    ...
    if (value && code === "r" && selectedRole) resetMatch();
    if (value && key === " ") handlePlayerInteraction(performance.now());
    if (value && code === "e") handlePlayerHeal(performance.now());
  ...
  function moveControlledHunter(dt) {
    ...
    hunter.status = hunter.target ? "controlled" : "allDowned";
  }
  ...
  function isAISafeToHeal(survivor, hunterDistance) {
    ...
    if (hunterDistance < 520) return false;
  ...
  function shouldKeepHealDecision(survivor, now) {
    ...
    return canBeHealed(decision.target) && isHealTargetSafe(decision.target);
  }
  ...
  function findBestAIHealTarget(healer) {
    ...
    getSurvivors().forEach((target) => {
      if (target === healer || !canBeHealed(target) || !isHealTargetSafe(target)) return;
      const statePriority = target.state === "downed" ? 0 : 1;
      const distance = distanceBetween(healer, target);
      if (distance > 980) return;
      const hunterDistance = distanceBetween(hunter, target);
      const score = statePriority * 1000 + distance - hunterDistance * 0.18;
      if (score < bestScore) {
        best = target;
        bestScore = score;
      }
  ...
  function isHealTargetSafe(target) {
    const hunterDistance = distanceBetween(hunter, target);
    if (hunterDistance < 420) return false;
    if (hunter.target === target && hunterDistance < 580) return false;
    return true;
  }
  ...
  function canBeHealed(survivor) {
    return isHealableState(survivor) && !survivor.action;
  }
  ...
  function cancelHealing(action) {
    if (!action) return;
    if (action.target && action.target.action && action.target.action.kind === "beingHealed") {
      action.target.action = null;
    }
    if (action.healer && action.healer.action && action.healer.action.kind === "healing") {
      action.healer.action = null;
    }
  }
  ...
  function updateActorAction(actor, now) {
    ...
    if (action.kind === "healing") {
      ...
      if (!isHealableState(action.target) || distanceBetween(actor, action.target) > 112) {
        cancelHealing(action);
        actor.action = null;
        return true;
      }
      if (now >= action.until) {
        finishHealing(action.target);
        actor.action = null;
      }
      return true;
    }
    ...
    if (action.kind === "beingHealed") {
      actor.vx = 0;
      actor.vy = 0;
      if (!action.healer.action || action.healer.action.kind !== "healing") {
        actor.action = null;
      }
      return true;
    }
  ...
  function drawTeammates() {
    teammates.forEach((survivor) => drawSurvivor(survivor, false));
  }
  ...
  function drawSurvivorLabel(survivor) {
    ...
    if (survivor.action && survivor.action.kind === "healing") {
      ...
      label = `治疗 ${Math.round(progress * 100)}%`;
    ...
    ctx.fillStyle = survivor.action && survivor.action.kind === "beingHealed"
      ? "#b7d6c1"
      : survivor.state === "healthy" ? "rgba(16, 22, 18, 0.86)" : survivor.state === "injured" ? "#d9b76a" : "#b95f52";
    ctx.beginPath();
    ctx.roundRect(-40, -9, 80, 18, 6);
    ctx.fill();
    ctx.fillStyle = survivor.state === "healthy" && (!survivor.action || survivor.action.kind !== "beingHealed") ? "#eef3ed" : "#101611";
  ...
  function drawPlayerInteractionHint() {
    ...
    const nearWindow = findNearestWindow(player, SURVIVOR_WINDOW_PROMPT_RANGE);
    let label = "";
    if (healTarget) label = `E 治疗${healTarget.name}`;
  ...
  function drawHunterInteractionHint(now) {
    ...
    const nearWindow = findNearestWindow(hunter, HUNTER_WINDOW_PROMPT_RANGE);
    let label = "";
    if (droppedPallet) label = "Space 踩板";
    else if (nearWindow) label = "Space 翻窗";
